...
- [x] 8.5 Attach `cresten.pizza` to the Worker, enforce valid HTTPS, optionally enable Cloudflare Web Analytics, and update/test the Content Security Policy if the beacon is enabled.
- [x] 8.6 Immediately smoke-test the apex-domain home, About, every legacy post and tag route, RSS, sitemap, headers, caching, and unknown-route status after cutover.
